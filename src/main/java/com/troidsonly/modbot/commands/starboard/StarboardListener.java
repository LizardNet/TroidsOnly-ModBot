...
public class StarboardListener extends ListenerAdapter {
    ...
    private synchronized void handleRegularMessage(GuildMessageReactionAddEvent event) {
        // TODO: probably a better approach than making this whole thing synchronous
        String messageId = event.getMessageId();
        String channelId = event.getChannel().getId();
        ...
        Message message = Objects.requireNonNull(event.getGuild().getTextChannelById(channelId))
        ...
        starboardChannel.sendMessage(
                        generateStarboardPreamble(starboardEmote, event.getChannel().getName(), reactionCount))
                .embed(generateStarboardEmbed(message))
                .complete();
        // TODO: Add the message state
    }

    private void handlePromotedMessage(GuildMessageReactionAddEvent event, StarboardMessageState messageState) {
        // TODO: Check if the message is tracked in state
        // TODO: Update emote count
    }
    ...
    private static String generateStarboardPreamble(PotentialEmote emote, String sourceChannelName, int reactCount) {
        StringBuilder sb = new StringBuilder("From #")
                .append(sourceChannelName)
                .append(" with ")
                .append(reactCount)
                .append(' ');
