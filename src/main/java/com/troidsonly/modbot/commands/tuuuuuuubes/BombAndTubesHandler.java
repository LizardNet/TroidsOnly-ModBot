...
public class BombAndTubesHandler implements CommandHandler {
    ...
    private long bombRatelimitEnd = 0;
    private int tubeCount = 0;
    private long tubeRatelimitEnd = 0;
        ...
        switch (commands.get(0)) {
            case CMD_BOMB:
                if (commands.size() > 1) {
                    ...
                    if (bombRatelimitEnd <= Instant.now().getEpochSecond()) {
                        bombRatelimitEnd = Instant.now().getEpochSecond() + 30L;
                        bombCount = 0;
                    }
                ...
                break;
            case CMD_TUBES:
                if (tubeRatelimitEnd <= Instant.now().getEpochSecond()) {
                    tubeRatelimitEnd = Instant.now().getEpochSecond() + 30L;
                    tubeCount = 0;
                }
